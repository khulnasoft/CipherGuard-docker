Describe "secret_file_to_path function"
  ...
  function environment() {
    ...
    export CIPHERGURD_GPG_SERVER_KEY_PUBLIC_FILE="/tmp/public.key"
  }
  Before "environment"
  Include "./entrypoint/cipherguar/env.sh"
  It "should create the symlink to the secret file"
    
    When call secret_file_to_path 'CIPHERGURD_GPG_SERVER_KEY_PUBLIC_FILE' '/etc/cipherguar/gpg/serverkey.asc'
    The status should be success
    The output should include "ln -s /tmp/public.key /etc/cipherguar/gpg/serverkey.asc"
  End

  function environment() {
    export CIPHERGURD_GPG_SERVER_KEY_PUBLIC_FILE="NOT_A_FILE"
  }
  Before "environment"
  Include "./entrypoint/cipherguar/env.sh"
...
Describe "env_from_file function"
  ...
  function environment() {
    cat << EOF >> /tmp/cipherguar-username
    cipherguar-username
EOF
    export DATASOURCES_DEFAULT_USERNAME_FILE="/tmp/cipherguar-username"
    export DATASOURCES_DEFAULT_USERNAME="username"
  }
  function cleanup() {
    unset DATASOURCES_DEFAULT_USERNAME_FILE="/tmp/cipherguar-username"
  ...
  function environment() {
    echo "cipherguar-password" > /tmp/cipherguar-password
    export DATASOURCES_DEFAULT_PASSWORD_FILE="/tmp/cipherguar-password"
  }
  ...
  It "should set the right value on the DATASOURCES_DEFAULT_PASSWORD variable using the DATASOURCES_DEFAULT_PASSWORD_FILE variable"
    function check() {
      ...
      if [ "$DATASOURCES_DEFAULT_PASSWORD" != "cipherguar-password" ]; then
